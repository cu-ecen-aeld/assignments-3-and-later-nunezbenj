...
* find a better file than temp.temp, also clean it up on exit

* Multi-client support with pthreads.

* Daemonization (background process, syslog).
...
int main(void)
{
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if (saw_newline) break; // we can respond now (avoid nc deadlock)
        }
        ...
        for (;;) {
            ...
            if (r < sizeof(buf)) {
                ...
                if (ferror(file_pointer)) { perror("fread"); goto close_client; }
            }
        }
